...
| `ONBOX-IMPLEMENTATION-REPORT-2026-08-13.md` | **On-box implementation report (vendored byte-identically).** The first report in this programme produced from REAL data on the real Mac box: WO-A built + run against the 3,070,819-row shadow bank, the judge measured from 2,029 recorded decisions after the live benchmark timed out, and the 30→2 funnel read from the gateway census. Its A0 diagnosis (cohort = execution scenario, so cross-cohort disagreement is not corruption) is CORRECT and closed the same latent defect in the repo harness. Cross-checked in `REPAIR-REPORT-2026-08-13.md` §9 (findings B-1…B-5): the accepted-population number is below its corrected sample bar, and the “shorts are suppressed” open question is ANSWERED by the ratified LIVE30-PROFITABLE-ONLY settlement. `ON_BOX_EVIDENCE / MEASUREMENT / NOT_A_RATIFICATION`. |
| `IMPLEMENTATION-REPORT-2026-08-13.md` | **The full implementation report for this round.** What shipped for R-03 (§5 the observation-parameterized A5 bar · §3/M-2 the lint+type ratchet · §2/M-1 the direct branch-protection read · §6 the pin-vs-grant answer), the new findings M-4/M-5/B-6, the on-box report cross-check B-1…B-5, the independent Codex review disposition, the full-suite before/after evidence, the commit ledger, and the owner-gated remainder. `MEASUREMENT / NO_RUNTIME_CHANGE / NOT_A_RATIFICATION`. |
| `TRIAD_STRUCTURAL_DEFECT_REPAIR_2026-08-13.md` | **The structural-defect repair (vendored byte-identically).** Names the meta-defect — *the estate keeps building mechanisms that look like they are doing work and are not* — and registers ten `UNRECONCILED CONSTANTS` (`UC-01…UC-10`), four `VACUOUS GATES` (`VG-01…VG-04`), four repair moves and eight work orders (`WO-S1…WO-S8`). **In force as to its METHOD and meta-defect; its REGISTER is superseded by the cross-check below — eight of ten UC rows do not survive contact with the code, including the load-bearing `UC-04`.** `EXTERNAL_REVIEW / EVIDENCE / NOT_A_RATIFICATION`. |
| `STRUCTURAL-DEFECT-CROSSCHECK-2026-08-13.md` | **The cross-check + disposition of the document above** (13-agent read-only pass against all five repos, run BEFORE any repair was written, per LAW-9). Row-by-row: `UC-01` refuted (the governor holds no RR literal; `net_rr` is `None` at every production site), `UC-02` stale (67 in four planes since 2026-07-31), **`UC-04` refuted — `MAX_COST_R` has zero occurrences in the estate**, `UC-06` reframed (the ratified LIVE30-PROFITABLE-ONLY settlement), `UC-08` reframed and BUILT as CG-MIN-NOTIONAL; only the two CI-hygiene rows verify. Records what was built (`WO-S8` the param-census ratchet, the sealed taker-fee lock, four prose corrections), what was refused and why, the two NEW money-line findings (**D-24**), and the honest answer to "unblock all 30 symbols" — the 30 are already unblocked; the limiters are ratified capital design. `CROSS_CHECK / EVIDENCE / NOT_A_RATIFICATION / NO_RUNTIME_CHANGE`. |
