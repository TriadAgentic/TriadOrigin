...
class CodeownersUnavailable(CodeownersError):
    """The fixed provider endpoint or authentication needed for proof is unavailable."""
...
def verify_provider(
    ...
    *,
    ...
    try:
        ...
        raise CodeownersUnavailable(
            f"CODEOWNERS_PROVIDER_IDENTITY_UNAVAILABLE:{exc}") from exc
    except LiveRulesetError as exc:
        raise CodeownersError(f"CODEOWNERS_PROVIDER_IDENTITY_MISMATCH:{exc}") from exc
...
def main(argv: list[str]) -> int:
    ...
    except CodeownersUnavailable as exc:
